...
# === Variables globales derivadas de env_config ===
PROJECT_ID = project_id
BQ_CONN_ID = "google_cloud_default"
BQ_LOCATION = region
BQ_DATASET = dataops_dataset
...
BQ_ENRICHED_TABLE = f"{PROJECT_ID}.{BQ_DATASET}.lookuptable_fk_pk_enriched"
BQ_LOOKUP_TABLE = f"{PROJECT_ID}.{BQ_DATASET}.lookuptable_fk_pk"
BQ_REL_TABLE = f"{PROJECT_ID}.{BQ_DATASET}.lookup_table_relationships"
...
def extract_and_process_metadata(**context):
    ...
    table_id = object_extraction_metadata_table
...
def run_sp_and_cleanup(**context):
    ...
    sp_sql_enriched = f"""
    CALL `{PROJECT_ID}.{BQ_DATASET}.sp_build_lookup_fk_pk_enriched`();
    """
    logging.info("Calling stored procedure sp_build_lookup_fk_pk_enriched...")
    client.query(sp_sql_enriched, location=BQ_LOCATION).result()
    ...
    """
    client.query(ddl_create_final, location=BQ_LOCATION).result()
    ...
    sp_sql_scores = f"""
    CALL `{PROJECT_ID}.{BQ_DATASET}.sp_enrich_pk_scores`();
    """
    logging.info("Calling stored procedure sp_enrich_pk_scores to compute scores...")
    client.query(sp_sql_scores, location=BQ_LOCATION).result()
...
def compute_lookup_flags(**context):
    ...
    """
    client.query(alter_is_lookup, location=BQ_LOCATION).result()
    ...
    """
    client.query(alter_lookup_class, location=BQ_LOCATION).result()
    ...
    WHERE TRUE
    """
    client.query(update_sql, location=BQ_LOCATION).result()
...
def check_lookup_candidates_and_log(**context):
    ...
    job = client.query(sql_candidates, location=BQ_LOCATION)
    rows = list(job.result())
...
# === DAG definition ===
with DAG(
    dag_id="bq_to_sqlserver_pk_catalog_pymssql_batches_cte_only_distinct_to_bq_v5",
